...
CCollision::CCollision(int nW, int nH) : CKhuGleWin(nW, nH) 
{
   ...
   m_AirResistance = CKgVector2D(0.1, 0.1); // 중력 공기저항 설정

   m_pScene = new CKhuGleScene(640, 480, KG_COLOR_24_RGB(100, 100, 150)); // 씬 설정

   m_pGameLayer = new CKhuGleLayer(600, 420, KG_COLOR_24_RGB(150, 150, 200), CKgPoint(20, 30)); // 레이어 설정
    
   m_pScene->AddChild(m_pGameLayer); // 씬에 레이어 추가
   ...
   m_pGameLayer->AddChild(m_pLine); //레이어에 추가

   m_pCircle1->m_Velocity = CKgVector2D(900, 50);
   m_pCircle2->m_Velocity = CKgVector2D(-100, -300); //기본 초기 속도 지정
   ...
   for(int i = 0 ; i < 100 ; i++)
   {
      ...
      m_pGameLayer->AddChild(m_pNewCircle[i]); //100개의 공 생성.
   }
}
...
void CCollision::Update()
{
   ...
   for(auto &Layer : m_pScene->m_Children) //레퍼런스로 해서 참조해옴.
   {
      for(auto &Sprite : Layer->m_Children)
      {
         ...
         if(CKgVector2D::abs(Ball->m_Velocity) < 0.01)
            Ball->m_Velocity = CKgVector2D(0, 0); // 속도의 오차값을 조정
      }
      ...
      for(auto &SpriteA : Layer->m_Children)
      {
         ...
         for(auto &SpriteB : Layer->m_Children)
         {
            ...
            if(Ball == Target) continue; //자기 자신인 경우 패스
            ...
            if(((CKhuGleSprite *)Target)->m_nType == GP_STYPE_ELLIPSE) 
            {
               ...
               if(Overlapped <= 0) //겹치거나 점충돌한 경우
               {
                  CollisionPairs.push_back({Ball, Target});
                  
                  if(CKgVector2D::abs(PosVec) == 0) // 겹친 경우 이동
                  {
