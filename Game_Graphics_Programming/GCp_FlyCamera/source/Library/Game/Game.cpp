...
LRESULT CALLBACK WindowProc(_In_ HWND hWnd, _In_ UINT uMsg, _In_ WPARAM wParam, _In_ LPARAM lParam)
{
    ...
    case WM_KEYUP: // ????? ? up
        ...
        return 0;
    case WM_RBUTTONDOWN: // ???²J ?? ??? down
        g_mouseRightClick = true;
        return 0;
    case WM_RBUTTONUP: // ???²J ?? ??? up
...
HRESULT InitDevice()
{
    ...
    for (UINT driverTypeIndex = 0; driverTypeIndex < numDriverTypes; driverTypeIndex++)
    {
        ...
        if (hr == E_INVALIDARG)
        {
            // Direct X 11.0?????? D3D_FEATURE_LEVEL_11_1?? ?¥í????? ???? ?? ????. ?? ??? ??? ???????.
...
void HandleInput(FLOAT deltaTime) {
    ...
    if (g_mouseRightClick) {
        camYaw -= g_mouseRelativeMovement.X * deltaTime * 0.37f;
        camPitch -= g_mouseRelativeMovement.Y * deltaTime * 0.37f;
    }
    if (g_inputDirections.bBack) {
        moveBackForward = -deltaTime * mass;
    }
    if (g_inputDirections.bDown) {
        eye += XMVectorSet(0.0f, mass * -deltaTime, 0.0f, 0.0f);
    }
    if (g_inputDirections.bFront) {
        moveBackForward = deltaTime * mass;
    }
    if (g_inputDirections.bLeft) {
        moveLeftRight = -deltaTime * mass;
    }
